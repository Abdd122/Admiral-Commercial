...
const AdminPanel = () => {
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'orders' | 'deposits' | 'codes' | 'reports' | 'notifications' | 'settings'>('products');
  const [products, setProducts] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    return () => { unsubP(); unsubO(); unsubC(); unsubN(); unsubD(); unsubS(); };
  }, [isAdmin]);
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-wrap gap-3 p-2 bg-slate-100/50 backdrop-blur-sm rounded-[2.5rem] w-fit border border-slate-200/60">
        {(['products', 'orders', 'deposits', 'codes', 'reports', 'notifications', 'settings'] as const).map(tab => {
          ...
          >
            <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900">إدارة الأصول الرقمية</h3>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">إضافة وتعديل المنتجات المتاحة في المتجر</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  {newProduct.packages.length > 0 && (
                    <div className="flex flex-wrap gap-3">
                      {newProduct.packages.map((pkg, i) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-xl shadow-sm">
                          <span className="text-xs font-bold text-slate-700">{pkg.name}</span>
                          <span className="text-xs font-black text-indigo-600 font-mono">${pkg.priceUSD}</span>
                          <button 
                ...
                <div className="md:col-span-4 flex items-end">
                  ...
                  >
                    ...
                    <span>حفظ المنتج النهائي مع كافة الحزم</span>
                  </button>
                </div>
              </div>
            </div>
                    ...
                    {orders.map(o => (
                      <tr key={o.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-10 py-6">
                          ...
                          {o.playerId && (
                            <p className="text-[10px] font-bold text-slate-500 mt-1 bg-slate-100 px-2 py-0.5 rounded-md inline-block">ID: {o.playerId}</p>
                          )}
                        </td>
                        <td className="px-10 py-6">
